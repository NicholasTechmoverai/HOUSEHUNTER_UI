<template>
  <UCard class="max-w-6xl mx-auto m-1 sm:m-2" :ui="{ body: { padding: 'p-0' } }">
    <!-- Header - Compact -->
    <div class="sticky top-0 z-20 bg-white dark:bg-gray-900 px-4 sm:px-6 py-3 border-b border-gray-200 dark:border-gray-800">
      <div class="flex items-center justify-between">
        ...
        <div class="hidden sm:block">
          <UButton color="primary" @click="savePricing" :disabled="!isFormValid" :loading="isSaving" size="sm" icon="i-heroicons-check">
            Save
          </UButton>
        </div>
      </div>
    ...
    <div class="p-3 sm:p-4 space-y-4 sm:space-y-6">
      ...
      <div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          ...
          <div class="space-y-1.5">
            <label class="block text-xs sm:text-sm font-medium">Deposit Type</label>
            <USelect v-model="priceForm.depositType" :items="depositTypeOptions" size="sm" />
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4">
        ...
        <div class="space-y-2">
          <div v-for="fee in basicAmenities" :key="fee.id" class="p-3 border border-gray-200 dark:border-gray-700 rounded-lg">
            <div class="flex items-center justify-between gap-2">
              ...
              <div class="flex items-center gap-2 flex-shrink-0">
                <div class="flex flex-col items-end">
                  <USwitch 
                    v-model="fee.is_included" 
                    @change="toggleAmenityInclusion(fee)"
                    size="sm"
      ...
      <div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4">
        ...
        <div v-if="additionalFees.length > 0" class="space-y-3">
          <div v-for="(fee, index) in additionalFees" :key="fee.id" class="p-3 border border-gray-200 dark:border-gray-700 rounded-lg">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-2">
                <div class="w-8 h-8 rounded-md bg-primary-50 dark:bg-primary-900/30 flex items-center justify-center flex-shrink-0">
                  <UIcon :name="getItemIcon(fee.name)" class="w-4 h-4 text-primary-600 dark:text-primary-400" />
                </div>
                <div class="min-w-0">
                  <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{{ fee.name || `Extra Fee #${index + 1}` }}</h4>
                  <p v-if="fee.description" class="text-xs text-gray-500 truncate">{{ fee.description }}</p>
                </div>
              </div>
              <UButton color="red" variant="ghost" size="xs" @click="removeAdditionalFee(fee.id)" icon="i-heroicons-trash" />
            </div>
            
            <div class="space-y-3">
              <UInput 
                v-model="fee.name" 
                placeholder="Name (e.g., Maintenance, Gym, etc.)" 
                size="xs" 
                class="text-sm"
              ...
              <!-- Included indicator -->
              <div v-if="fee.is_included" class="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-900/20 p-2 rounded-lg border border-emerald-200 dark:border-emerald-800">
        ...
        <div v-else class="text-center py-4 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
          <div class="w-12 h-12 rounded-full bg-primary-50 dark:bg-primary-900/30 flex items-center justify-center mx-auto mb-2">
