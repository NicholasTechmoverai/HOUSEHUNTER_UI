    ...
    <UCard class="border border-gray-200 dark:border-gray-800">
      ...
      <div class="mb-6 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
        <div class="flex items-center justify-between">
          <UInput v-if="allowEditEmail" trailing-icon="i-lucide-at-sign" placeholder="Enter your email" size="md" :value="email"
            :disabled="disabled"  />
      ...
      <div class="space-y-4">
        <div>
          ...
          <UPinInput v-model="code" :disabled=" loading" :autofocus="true" :length="6" placeholder="•"
            class="justify-center" :ui="{
        ...
        <div class="flex flex-col sm:flex-row gap-3 pt-2">
          <UButton type="button" :disabled="!allowVerify || loading" :loading="loading" loading-icon="i-lucide-loader" icon="i-heroicons-check" color="primary" variant="solid"
            class="flex-1" @click="handleSubmit">
            Verify Email
          </UButton>

          <UButton type="button" :disabled="resendCooldown > 0" color="gray" variant="ghost" icon="i-heroicons-arrow-path"
            @click="handleResend" class="flex-1">
            {{ resendButtonText }}
          </UButton>
        </div>
      </div>
...
const allowEditEmail = ref(props.disabled ? false : true)
...
watch(() => props.g_loading, (newLoading) => {
...
const handleSubmit = async () => {
  ...
  try {
    emit('submit', code.value,props.email)
  } catch (err) {
